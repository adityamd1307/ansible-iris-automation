...
| Interop production class import | **ObjectScript (guarded)** | Classes must be loaded on primary only; code DB mirrors to backup | `Ens.Director` existence check; `LOAD` with `ck`; skip if interop absent | `objectscript/import_production_classes.cos.j2`, `roles/iris_production/tasks/import.yml`, `playbooks/setup_production.yml` (primary play) |
| Interop production auto-start | **ObjectScript (guarded)** | Operational setting via `Ens.Director`; not mirrored | `GetAutoStartProduction()` checked first, `SetAutoStart()` only if different; applied on **all nodes** | `objectscript/setup_production.cos.j2`, `roles/iris_production/tasks/autostart.yml`, `playbooks/setup_production.yml` (all-nodes play) |
...
| Mirror member config | **CPF merge** | Declarative mirror settings | `[Config.Mirror]` merge per role | `cpf/mirror-template.cpf.j2`, `playbooks/setup_mirror.yml` |
| Mirror create / join / failover | **ObjectScript (guarded)** | Runtime mirror APIs; order matters | Primary create+DBs → backup join → primary `AddFailoverMember`; `#2137` treated as exists | `objectscript/setup_mirror_primary.cos.j2`, `setup_mirror_backup.cos.j2`, `setup_mirror_primary_add_failover.cos.j2`, `playbooks/setup_mirror.yml` |
| Web Gateway CSP routing | **File seed (`prepare.yml`)** | Portal must reach IRIS via superserver | Bryan-style `CSP.ini` (port 1972, `CSPSystem`); stale `webgateway*/durable` cleared on prepare | `config/webgateway/CSP.ini.j2`, `config/webgateway/CSP.conf`, `playbooks/prepare.yml` |
